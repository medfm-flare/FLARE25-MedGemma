...
import torch
# Fix compilation errors with Gemma3 attention mechanism
...
from transformers import AutoTokenizer, AutoProcessor, AutoModelForImageTextToText, GenerationConfig
    ...
    if "classification" in task_type_normalized and "multi" not in task_type_normalized:
        ...
        try:
            ...
            try:
                ...
                logger.debug("Successfully imported advanced metrics from utils.metrics")
            except ImportError as e:
                logger.warning(f"Advanced metrics not available ({e}), using simplified version")
...
def create_medgemma_prompt(row):
    """Create evaluation prompt for MedGemma format - EXACTLY matching training format"""
    # Handle NaN task_type values (same logic as training)
    ...
    num_images = 1  # MedGemma uses single image
    
    # EXACT SAME instructions as training
    ...
    # EXACT SAME content order as training: images first, then text
    content = []
...
def evaluate_model(args):
    try:
        ...
        logger.info("="*60)
        logger.info("STARTING MEDGEMMA MODEL EVALUATION ON VALIDATION SET")
        logger.info("="*60)
        ...
        # Test GREEN score import early
        try:
            from utils.metrics import calculate_green_score, calculate_bleu_score, calculate_clinical_efficacy_score
            logger.info("✅ Advanced metrics (GREEN score) successfully imported from utils.metrics")
        except ImportError as e:
            logger.warning(f"⚠️  Advanced metrics not available ({e}), will use simplified fallback")
        ...
        for model_config in models_to_evaluate:
            ...
            if model_config['use_lora']:
                ...
                model = AutoModelForImageTextToText.from_pretrained(
                    ...
                    attn_implementation="eager"  # Use eager attention to avoid compilation errors
                )
                # Load LoRA weights
            ...
            # Configure cache settings to prevent cache_size_limit errors
            if hasattr(model, 'generation_config'):
                if not hasattr(model.generation_config, 'cache_implementation'):
                    from transformers import GenerationConfig
                    model.generation_config = GenerationConfig.from_model_config(model.config)
                model.generation_config.cache_implementation = "dynamic"
                # Try to increase cache size limits if available
                if hasattr(model.generation_config, 'max_cache_length'):
                    model.generation_config.max_cache_length = 8192  # Increase cache size
                logger.info("Configured model to use dynamic cache implementation")
            
            # Additional cache configuration at model level if available
            if hasattr(model.config, 'cache_config'):
                model.config.cache_config = {"cache_implementation": "dynamic"}
                logger.info("Set cache configuration at model config level")
            
            # Load tokenizer and processor separately for MedGemma
            ...
            for task_type, all_task_examples in task_groups.items():
                ...
                for i in tqdm(range(0, len(current_task_examples), current_batch_size), desc=f"Evaluating {task_type} with {model_config['name']}"):
                    # Clear cache at the start of each batch to prevent accumulation
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
                    ...
                    for example in batch_examples:
                        try:
                            # Load images - handle both single and multi-image cases
                            example_images = []
                            if "images" in example:
                                # New multi-image format
                                for img_path in example["images"]:
                                    image = Image.open(img_path).convert("RGB")
                                    # Resize to appropriate size for MedGemma 
                                ...
                                logger.error(f"No image field found in example")
                                continue
                            
                            # For MedGemma, we process the first image (MedGemma doesn't support multi-image)
                            primary_image = example_images[0]
                            batch_images_pil.append(primary_image)
                            
                            # Create MedGemma-style prompt
                            prompt_data = create_medgemma_prompt(example)
                            
                            batch_prompts.append(prompt_data)
                            
                            if len(example_images) > 1:
                                logger.warning(f"MedGemma doesn't support multi-image. Using only the first image from {len(example_images)} images.")
                    ...
                    try:
                        # Process each prompt individually for MedGemma (since we have batch_size=1)
                        for j, prompt_data in enumerate(batch_prompts):
                            try:
                                # Apply chat template for MedGemma - SAME as training
                                ...
                                # Process with processor like training
                                inputs = processor(
                                    images=[batch_images_pil[j]],  # Single image
                                    text=full_text,
                                    return_tensors="pt",
                                    padding=True,
                                    truncation=False,
                                ...
                                # Generate response - MedGemma specific parameters
                                gen_kwargs = {
                                    "max_new_tokens": 200,  # Increased for complete responses
                                    "do_sample": False,     # Deterministic generation
                                    "use_cache": True,      # Enable caching
                                    "cache_implementation": "dynamic"  # Use dynamic cache implementation
                                }
                                
                                # Increase tokens for instance detection
                                if task_type == "instance_detection":
                                    gen_kwargs["max_new_tokens"] = 800  # Reduced from 1000
                                elif task_type == "report_generation":
                                    gen_kwargs["max_new_tokens"] = 300  # Slightly more for reports
                                
                                # Clear cache before generation to prevent accumulation
                                if hasattr(model, 'generation_config'):
                                ...
                                try:
                                    ...
                                    if "cache_size_limit" in str(cache_error).lower():
                                        logger.warning(f"Cache limit reached, trying with reduced tokens for task {task_type}")
                                        # Retry with much smaller token limit
                                        gen_kwargs["max_new_tokens"] = min(100, gen_kwargs["max_new_tokens"] // 2)
                                        gen_kwargs["use_cache"] = False  # Disable cache for problematic cases
                                        ...
                                        try:
                                            ...
                                            logger.error(f"Generation failed even with reduced tokens: {retry_error}")
                                            task_skipped_count += 1
                                            continue
                                    else:
                                        logger.error(f"Generation error: {cache_error}")
                                        task_skipped_count += 1
                                        continue
                                ...
                                # Store the result
                                outputs_batch_decoded = [decoded_output]  # Make it a list for compatibility
                                
                                # Process the single output
                                break  # Since we're processing one at a time
                            except Exception as e_single:
                                logger.error(f"Error processing single prompt: {e_single}")
                                task_skipped_count += 1
                                outputs_batch_decoded = []
                                continue
            ...
            # Multiple rounds of cache clearing for thorough cleanup
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
                torch.cuda.ipc_collect()  # Clear IPC cache as well
        ...
        total_eval_time = datetime.datetime.now() - eval_start_time
        logger.info(f"Total evaluation time: {total_eval_time}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate MedGemma model on validation set")
    ...
    parser.add_argument("--evaluate_base_model", action="store_true", default=False, help="Evaluate the base model for comparison (default: False for MedGemma)")
    parser.add_argument("--skip_base_model", action="store_true", help="Skip base model evaluation (only evaluate fine-tuned model)")
    parser.add_argument("--base_model_path", type=str, default="google/medgemma-4b-it", help="Path to the base model")
    ...
    elif args.evaluate_base_model:
        logger.info("Base model evaluation enabled for comparison")
    else:
        logger.info("Base model evaluation disabled (default for MedGemma)")
